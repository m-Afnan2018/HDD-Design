...
const ProductArea = () => {
  ...
  if (!isLoading && !isError && products?.data?.length > 0) {
    ...
    if (activeTab === 'All Collection') {
      ...
      product_items = products.data.filter(p => p.children === 'Shoes' || p.children?.toLowerCase().includes('shoe'))
    } else if (activeTab === 'Clothing') {
      product_items = products.data.filter(p => p.parent === 'Clothing' || p.children === 'T-Shirts' || p.children?.toLowerCase().includes('cloth'))
    } else if (activeTab === 'Bags') {
      product_items = products.data.filter(p => p.children === 'Bags' || p.children?.toLowerCase().includes('bag'))
    } else {
      product_items = products.data;
    }
    content = <>
