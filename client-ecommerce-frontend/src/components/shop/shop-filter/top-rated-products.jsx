...
const TopRatedProducts = () => {
  ...
  if (isLoading) {
    ...
    content = product_items.map((item) => {
      const defaultVariant = getDefaultVariant(item.imageURLs);
      const hasValidImage = defaultVariant?.img && defaultVariant.img.trim() !== '';
      const imageSrc = hasValidImage ? defaultVariant.img : '/assets/img/error/error.png';
      ...
      <div key={item._id} className="tp-shop-widget-product-item d-flex align-items-center">
        <div className="tp-shop-widget-product-thumb">
          <Link href={`/product-details/${item._id}`}>
            <Image src={imageSrc} alt="product img" width={70} height={70} />
          </Link>
        </div>
