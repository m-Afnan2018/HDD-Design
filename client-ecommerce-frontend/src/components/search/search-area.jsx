...
export default function SearchArea() {
  ...
  const productType = searchParams.get('productType') || 'fashion';
  ...
  if (!isLoading && !isError && products?.data?.length > 0) {
    ...
    if (searchText) {
      product_items = all_products
        .filter((prd) => prd.productType?.toLowerCase() === productType.toLowerCase())
        .filter((prd) => prd.title.toLowerCase().includes(searchText.toLowerCase()));
    } else {
      product_items = all_products.filter((prd) => prd.productType?.toLowerCase() === productType.toLowerCase());
    }
     // Price low to high
