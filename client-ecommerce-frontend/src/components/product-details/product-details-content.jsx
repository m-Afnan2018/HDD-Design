...
const ProductDetailsContent = ({ productItem }) => {
  const { _id, imageURLs, videoId,status } = productItem || {};
  const default_item = getDefaultVariant(imageURLs || []);
  const [activeImg, setActiveImg] = useState(default_item?.img || "");
  const dispatch = useDispatch();
  // active image change when img change
  useEffect(() => {
    setActiveImg(default_item?.img || "");
  }, [default_item]);
