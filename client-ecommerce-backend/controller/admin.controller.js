...
const loginAdmin = async (req, res,next) => {
  try {
    ...
    // const ADMIN_EMAIL = 'mr.surah12345@gmail.com';
    // const ADMIN_PASSWORD = 'danish12345';
    ...
    // Create admin data without database lookup
    const adminData = {
      _id: "admin_id",
      name: "Super Admin",
      email: ADMIN_EMAIL,
...
const forgetPassword = async (req, res,next) => {
  try {
    ...
    // Only allow password reset for the hardcoded admin email
    if (email !== ADMIN_EMAIL) {
      return res.status(403).send({
        message: "Password reset is only available for authorized admin email.",
      });
    }
    ...
    if (!admin) {
      ...
      const body = {
        ...
        <p>A request has been received to change the password for your <strong>HD design Fashion Hub</strong> admin account </p>

        <p>This link will expire in <strong> 10 minute</strong>.</p>

        <p style="margin-bottom:20px;">Click this link for reset your password</p>

        <a href=${secret.admin_url}/forgot-password/${token} style="background:#0989FF;color:white;border:1px solid #0989FF; padding: 10px 15px; border-radius: 4px; text-decoration:none;">Reset Password</a>

        <p style="margin-top: 35px;">If you did not initiate this request, please contact us immediately at support@hddesignfashionhub.com</p>

        <p style="margin-bottom:0px;">Thank you</p>
        <strong>HD design Fashion Hub Team</strong>
        `,
      };
