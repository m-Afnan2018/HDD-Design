...
const DetailsWrapper = ({ productItem, handleImageActive, activeImg, detailsBottom = false }) => {
  const { sku, title, imageURLs, description, discount, price, status, tags, offerDate, category, parent, reviews } = productItem || {};
  // Use category.name if available, otherwise fall back to parent
  const categoryName = category?.name || parent || 'Product';
