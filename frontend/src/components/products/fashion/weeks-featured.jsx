    ...
    content = (
      <Swiper {...slider_setting} modules={[Navigation]} className="tp-featured-slider-active swiper-container">
        {product_items.map((item) => {
          const {_id, title, price, discount, reviews } = item || {};
          const default_item = getDefaultVariant(item.imageURLs || []);
          return (
            <SwiperSlide key={item._id} className="tp-featured-item white-bg p-relative z-index-1">
              <div className="tp-featured-thumb include-bg" style={{ backgroundImage: `url(${default_item?.img})` }} data-background="assets/img/product/slider/product-slider-1.jpg"></div>
